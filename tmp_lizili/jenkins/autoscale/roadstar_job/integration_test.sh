...
if [ ! -z "$HOSTNAME" ];then
  LOG_BACK_UP_PATH="$LOG_BACK_UP_PATH/$TODAY/$HOSTNAME"
else
  LOG_BACK_UP_PATH="$LOG_BACK_UP_PATH/$TODAY"
fi
mkdir -p $LOG_BACK_UP_PATH
...
function backup_log() {
  if [ -d "$ROADSTAR_ROOT_DIR/release/data/log" ];then
    ...
    echo "Backup release log successfully. log path $LOG_BACK_UP_PATH"
  else
...
MASTER_COMMIT_ID=`cat release/meta.ini | grep git_commit`
