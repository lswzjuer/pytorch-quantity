...
DATE=$(date +%F)
ROS_LOG_DIR="${ROADSTAR_ROOT_DIR}/data/log/${DATE}/leopard_camera"
LOG="${ROADSTAR_ROOT_DIR}/data/log/${DATE}/leopard_camera.log"
...
function start() {
  ...
  CMD="roslaunch usb_cam ${launch_file}.launch"
...
function start_fe() {
  ...
  CMD="ROS_LOG_DIR=${ROS_LOG_DIR} roslaunch usb_cam ${launch_file}.launch"
  NUM_PROCESSES="$(pgrep -c -f "${launch_file}")"
  if [ "${NUM_PROCESSES}" -eq 0 ]; then
    eval "${CMD}"
  fi
...
function start_with() {
  ...
  CMD="roslaunch usb_cam ${launch_file}.launch ${param}"
  NUM_PROCESSES="$(pgrep -c -f "${launch_file}")"
  if [ "${NUM_PROCESSES}" -eq 0 ]; then
    eval "ROS_LOG_DIR=${ROS_LOG_DIR} nohup ${CMD} </dev/null >${LOG} 2>&1 &"
  fi
...
function start_fe_with() {
  ...
  CMD="ROS_LOG_DIR=${ROS_LOG_DIR} roslaunch usb_cam ${launch_file}.launch ${param}"
  NUM_PROCESSES="$(pgrep -c -f "${launch_file}")"
  if [ "${NUM_PROCESSES}" -eq 0 ]; then
    eval "${CMD}"
  fi
