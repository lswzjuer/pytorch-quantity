...
#ifndef MODULES_COMMON_CANBUS_CAN_COMM_CAN_SENDER_H_
#define MODULES_COMMON_CANBUS_CAN_COMM_CAN_SENDER_H_
 ...
 */
using canbus::CanClient;
using canbus::CanFrame;
...
class SenderMessage {
 public:
  ...
  /**
   * @brief Update watch dog
   */
  void UpdateWatchdog(uint8_t *data);
...
template <typename SensorType>
void SenderMessage<SensorType>::UpdateWatchdog(uint8_t *data) {
  protocol_data_->UpdateWatchdogCounter(data);
}
...
void CanSender<SensorType>::PowerSendThreadFunc() {
  ...
  while (is_running_) {
    ...
    for (auto &message : send_messages_) {
      ...
      message.UpdateWatchdog(can_frame.data);
...
template <typename SensorType>
bool CanSender<SensorType>::Send() {
  for (auto &message : send_messages_) {
    std::vector<CanFrame> can_frames;
    CanFrame can_frame = message.CanFrame();
