...
ErrorCode EsdCanClient::Receive(std::vector<CanFrame> *const frames,  // NOLINT
                                int32_t *const frame_num) {
  if (!is_started_) {
  ...
  for (int32_t i = 0; i < *frame_num && i < kMaxCanRecvFrameLen; ++i) {
    CanFrame cf;
...
std::string EsdCanClient::GetErrorString(const NTCAN_RESULT ntstatus) {
  ...
  static const struct ERR2STR kErr2Str[] = {
  ...
  const struct ERR2STR *es = kErr2Str;
