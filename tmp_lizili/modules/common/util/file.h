...
template <typename MessageType>
bool AddProtoToASCIIFile(const MessageType &message,
                         const std::string &file_name) {
  return SetProtoToASCIIFile(
      message, open(file_name.c_str(), O_WRONLY | O_CREAT | O_APPEND, S_IRWXU));
}
...
template <typename MessageType>
bool AddProtoToBinaryFile(const MessageType &message,
                          const std::string &file_name) {
  std::fstream output(file_name,
...
template <typename MessageType>
bool GetProtoFromString(const std::string &input, MessageType *message) {
  using google::protobuf::TextFormat;
  return TextFormat::ParseFromString(input, message);
}
...
bool isFile(const std::string &path);  // NOLINT

bool isDir(const std::string &path);  // NOLINT
