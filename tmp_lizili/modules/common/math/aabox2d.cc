...
double AABox2d::GetOverlap(const AABox2d &box, const bool &is_union) const {
  const double area_1 = length_ * width_;
  const double area_2 = box.length() * box.width();
  const double h = std::min(max_y(), box.max_y()) - std::max(min_y(), box.min_y());
  const double w = std::min(max_x(), box.max_x()) - std::max(min_x(), box.min_x());
  if (w <= 0 || h <= 0) {
    return 0.0f;
  }
  const double area_o = w * h;
  if (is_union) {
    return area_o / (area_1 + area_2 - area_o);
  } else {
    return area_o / area_1;
  }
}
