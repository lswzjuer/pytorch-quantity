...
TEST(GPADQpSolverTest, simpleproblem) {
  MatrixXd kernel_matrix = MatrixXd::Zero(1, 1);
  kernel_matrix(0, 0) = 1.0;
  MatrixXd offset = MatrixXd::Zero(1, 1);
  offset(0, 0) = -8.0;
  MatrixXd affine_inequality_matrix = MatrixXd::Zero(1, 1);
  MatrixXd affine_inequality_boundary = MatrixXd::Zero(1, 1);
  MatrixXd affine_equality_matrix = MatrixXd::Zero(1, 1);
  MatrixXd affine_equality_boundary = MatrixXd::Zero(1, 1);
  int max_iter = 1000;
  double eps = 1e-9;
  GPADQpSolver solver(kernel_matrix, offset, affine_inequality_matrix,
