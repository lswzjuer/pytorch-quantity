...
TEST(Box2dTest, AreaOfIntersection) {
  EXPECT_EQ(box1.AreaOfIntersection(box2), 0.0);
  EXPECT_EQ(box1.AreaOfIntersection(box3), 0.0);
  EXPECT_EQ(box1.AreaOfIntersection(box4), 0.0);
  EXPECT_EQ(box2.AreaOfIntersection(box4), 0.0);
  EXPECT_EQ(box3.AreaOfIntersection(box4), 0.0);

  EXPECT_EQ(box1.AreaOfIntersection(box1), 8.0);
  Box2d rotate_box1 = box1;
  rotate_box1.RotateFromCenter(M_PI_2);
  EXPECT_EQ(box1.AreaOfIntersection(rotate_box1), 4.0);

  Box2d b1({-6.3 / 2, 0}, 0, 6.3, 2.6);
  Box2d b2({-5.23, -1.26}, 0.29, 4.53, 2.05);
  std::cout << b1.AreaOfIntersection(b2) << std::endl;
}
