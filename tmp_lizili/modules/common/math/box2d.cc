...
double Box2d::AreaOfIntersection(const Box2d &box) const {
  if (!HasOverlap(box)) {
    return 0;
  }
  ...
  auto GetBoostPolygon = [](const Box2d &box) {
    BoostPolygon polygon;
    const auto &corners = box.GetAllCorners();
    for (const auto &corner : corners) {
      boost::geometry::append(polygon.outer(),
                              BoostPoint(corner.x(), corner.y()));
    }

    return polygon;
  };
  BoostPolygon this_box_polygon = GetBoostPolygon(*this);
  BoostPolygon other_box_polygon = GetBoostPolygon(box);

  std::vector<BoostPolygon> output;
  boost::geometry::union_(this_box_polygon, other_box_polygon, output);
  double union_area = 0;
  for (const auto &p : output) union_area += boost::geometry::area(p);

  return this->area() + box.area() - union_area;
}
