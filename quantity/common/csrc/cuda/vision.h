...
namespace cuda {
int nms(int batch_size, const void* const* inputs, void** outputs, size_t count,
        int detections_per_im, float nms_thresh, void* workspace,
        size_t workspace_size, cudaStream_t stream);

int decode(int batch_size, const void* const* inputs, void** outputs,
           size_t height, size_t width, size_t scale, size_t num_anchors,
           size_t num_classes, const std::vector<float>& anchors,
           float score_thresh, int top_n, void* workspace,
           size_t workspace_size, cudaStream_t stream);
...
at::Tensor ROIPool_backward_cuda(
    const at::Tensor& grad, const at::Tensor& input, const at::Tensor& rois,
...
at::Tensor compute_flow_cuda(const at::Tensor& boxes, const int height,
                             const int width);
